...
- (void)viewDidLoad
{
    ...
    [button setTitle:NSLocalizedString(@"native调js", nil) forState:UIControlStateNormal];
    [button addTarget:self action:@selector(nativeTojs) forControlEvents:UIControlEventTouchUpInside];
    ...
    _weexHeight = self.view.frame.size.height - 64 - 200;
...
- (void)nativeTojs {
    ...
    [_instance fireGlobalEvent:@"sayhello" params:@{@"key":@"hello js,I'm native"}];
...
- (void)render
{
    ...
    _instance.frame = CGRectMake(self.view.frame.size.width-width, 200, width, _weexHeight);
